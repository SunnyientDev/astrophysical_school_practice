...
##            print(p)
        ...
        print(p[3])
        features.append([p[3], np.array(Lines)[0,1]])
        currentAxis = matplotlib.pyplot.gca()
        currentAxis.plot(p[3], np.array(Lines)[0,1], 'go')   
        currentAxis.annotate('{0:.2f}'.format(p[3]), xy=(p[3], np.array(Lines)[0,1]-0.02),rotation=-90,horizontalalignment='center',  verticalalignment='top',  color='g')
...
##    global Lines
    if (X>min(np.array(Continuum)[:,0]) and X<max(np.array(Continuum)[:,0])):
        if len(Lines)>0: Lines[0] = [X, Y]
        else: Lines.append([X,Y])
        matplotlib.pyplot.gca()
