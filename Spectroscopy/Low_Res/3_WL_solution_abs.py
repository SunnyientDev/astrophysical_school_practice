...
##            print (y_coo)
            currentAxis.annotate(WL, xy=(x_coo,y_coo),rotation=270,horizontalalignment='center',  verticalalignment='right',  color='g')
    Connect(fig_pic)
...
##    features=([20, 8000],[51, 7500],[89.6, 7000],[128.3, 6500],[182.5, 6000],[256, 5500],[345, 5000],[484.2, 4500],[670, 4000])
    ...
    params = np.zeros(1+int(Order_E.get()))
    
    errorfunction = lambda C: np.ravel(solution(C)(X) - W)
    C, success = optimize.leastsq(errorfunction, params, maxfev=10000)
    W_sol = solution(C)(X)
    W_full = solution(C)(X_full)
    resudial = W-W_sol
    rms = round(np.std(resudial),5)
    label_data = 'Chebyshev,' + ' points='+ str(X.shape[0])  + ', RMS(A)=' + str(rms)
    ...
    if Mode=='R':
        Pic = 'R'
        currentAxis.plot(X, resudial, 'go')
        currentAxis.set_xlabel("Position(pix)")
        currentAxis.set_ylabel("WL resudial(A)")
    if Mode=='F':
        Pic = 'F'        
...
def Add_solution(_Data, _Header, _Name):
    C = fit('F')
##    print(C)
    X = np.arange(0, _Data.shape[0], 1)
    WL = solution(C)(X)
##    print(WL)
    w1=np.min(solution(C)(0))
    w2=np.max(solution(C)(_Data.shape[0]))
    nw=len(X)
    nw = nw*int(Rebin_E.get())
    dw=(w2-w1)/(nw-1)
    print ('Blue end:', np.min(WL), 'Red end:', np.max(WL), 'A/pix:', dw, 'Total length:', nw)
    
    if (dw<0):
##        WL_lin=np.linspace(w2,w1, nw)
        WL_rebin=np.linspace(w2,w1, nw)
        Y = _Data[::-1]
        WL = WL[::-1]
    else:
##        WL_lin=np.linspace(w1,w2, nw)
        WL_rebin=np.linspace(w1,w2, nw)
        Y = _Data

    ##rebin X10
    rebin_func = interpolate.splrep(WL, Y, s=0, k=interp_order) 
    Data_rebin = interpolate.splev(WL_rebin, rebin_func, der=0)
            
    fig_pic = matplotlib.pyplot.figure(2)
    matplotlib.pyplot.cla()
    currentAxis = matplotlib.pyplot.gca()
    
    currentAxis.plot(WL, Y, 'r') ##plot old spec
    currentAxis.plot(WL_rebin, Data_rebin, 'g') ##plot rebin spec
##    currentAxis.plot(Y, 'r') ##plot new spec
##    currentAxis.plot(Data_rebin, 'g') ##plot new spec

    matplotlib.pyplot.show()
    
##write data to file
    save_to_file(Data_rebin, _Name, _Header, np.min(WL_rebin), abs(dw))

#################################################################################
def save_to_file(_data, _name, _header, CRVAL, CDELT):
##        Header['WCSDIM'] = 1    
##        Header['LTM1_1'] = 1
...
def get_max(x_coo):
    ...
    j = ROI.argmin() #get maximum pixel
...
def mark_feature(x_coo):
    ...
    features.append([x_coo, float(WL), _min])
    features = sorted(features)
    
    currentAxis = matplotlib.pyplot.gca()
    currentAxis.plot(x_coo, _min, 'go')   
    currentAxis.annotate(WL, xy=(x_coo,_min),rotation=270,horizontalalignment='center',  verticalalignment='left',  color='g')
    matplotlib.pyplot.draw()
